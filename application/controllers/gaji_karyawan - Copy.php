...
class gaji_karyawan extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->data = array();
        init_generic_dao();
        $this->load->model(array('m_gaji_karyawan', 'm_karyawan', 'm_penempatan','m_penagihan'));
    ...
    private function fetch_input() {
        ...
        if ($bpjstk_status == 1) {
            $hitung_bpjstk = $gapok * 0.624;
            $potong_bpjstk = $gapok * 0.924;
        } else {
            $hitung_bpjstk = $gapok * 0.624;
            $potong_bpjstk = $gapok * 0.624;
        }

        if ($bpjskes_status == 1) {
            $hitung_bpjskes = $gapok * 0.4;
            $potong_bpjskes = $gapok * 0.5;
        } else {
            $hitung_bpjskes = $gapok * 0.4;
            $potong_bpjskes = $gapok * 0.4;
        }
        ...
        //hitung ptkp
        $biaya_jabatan = $hitungthp * 0.05;       //hitung biaya jabatan
        $biaya_jabatan2 = $hitungthp - $biaya_jabatan;        //hitung biaya jabatan
    ...
    public function add() {
        // $data2 = $this->fetch_input2();
        // $data2['created_by'] = $this->session->userdata('username');
    ...
    public function edit($id_gaji_karyawan) {
        // $data2 = $this->fetch_input2();
        // $data2['updated_by'] = $this->session->userdata('username');
        ...
        if ($this->validate() != false) {
            ...
            // $this->m_penagihan->update($data2, $data_id);
            redirect(CURRENT_CONTEXT);
        } else {
            $this->preload();
            $this->data['edit'] = true;
    ...
    public function export_excel(){
        ...
        if($penempatan != 0) {      //data gaji berdasarkan penempatan
            ...
            $objWriter->save('./assets/excel/Data Gaji.xlsx'); 

            $this->load->helper('download');
            force_download('./assets/excel/Data Gaji.xlsx', NULL);
    ...
    public function input_gaji(){
        ...
        // $this->data['getGaji'] = $this->m_gaji_karyawan->getGaji($bulan, $tahun);
        // $this->data['getKaryawan'] = $this->m_gaji_karyawan->getKaryawan();
        $created_by = $this->session->userdata('username');
        $created_at = date('Y-m-d H:i:s');
        $tgl_awal = $this->input->post('awal_tgl');
        $tgl_akhir = $this->input->post('akhir_tgl');
        ...
        $get_penagihan = $this->data['getPenagihan'] = $this->m_gaji_karyawan->getPenagihan();
        foreach ($get_penagihan as $getp) {

            $data4 = array(
                    'id_karyawan' => $getp->id_karyawan,
                    'id_penempatan' => $getp->id_penempatan,
                    'gaji_id' => $id_gaji2++,
                    'nama_karyawan' => $getp->nama_karyawan,
                    // 'id_gaji_karyawan' => $getp->id_gaji_karyawan,
                    'nip' => $getp->nip,
                    'periode' => $tgl_akhir,
                    'upah' => $getp->upah,
                    'fee_bulan' => $getp->fee_bulan,
                    'rafel' => $getp->rafel,
                    'thr' => $getp->thr,
                    'pph23' => $getp->pph23,
                    'jml_tagihan' => $getp->jml_tagihan,
                    "created_at"=> $created_at,
                    "created_by"=> $created_by
                );
            $this->db->insert("penagihan",$data4);
